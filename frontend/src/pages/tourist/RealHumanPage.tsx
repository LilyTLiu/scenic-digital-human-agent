...
export default function RealHumanPage() {
  ...
  // 本地音频静默播放，仅用 currentTime 驱动字幕同步（用户听到的是 WebRTC 音频）
  const playSynced = useCallback((text: string, audioBlob: Blob) => {
    stopPlayback()
    const url = URL.createObjectURL(audioBlob)
    const audio = new Audio(url)
    audio.muted = true   // muted 允许浏览器自动播放（比 volume=0 更可靠触发 onplay）
    audioRef.current = audio
    setSubtitle('')

    let started = false
    const startTimeline = (durationMs: number) => {
      if (started) return
      started = true
    ...
    audio.onplay = () => {
      if (audio.duration && audio.duration > 0) {
        startTimeline(audio.duration * 1000)
      }
    }

    // 兜底：如果 onplay 始终不触发（浏览器策略），用 loadedmetadata 或超时启动
    audio.onloadedmetadata = () => {
      if (!started && audio.duration && audio.duration > 0) {
        // 有些浏览器在 muted 状态下不触发 onplay 但会触发 loadedmetadata
        startTimeline(audio.duration * 1000)
      }
    }

    // 最终兜底：500ms 后还没开始就按文本长度估算
    setTimeout(() => {
      if (!started) {
        startTimeline(text.length * 250)
      }
    }, 500)
  ...
  const sendToAI = useCallback(async (text: string) => {
    try {
      ...
      const res = await chatApi.send({ message: text, scenic_spot: '灵山胜境',
        persona_name: persona.name, persona_role: persona.role, persona_style: persona.style })
      const reply = res.reply || ''
      setStatusText('')

      // TTS → 数字人口型(WebRTC静音) + 本地音频播放 + 字幕精准同步
      try {
        const audioBlob = await voiceApi.tts(reply, persona.voice,
          (persona as any).tts_style, (persona as any).tts_rate, (persona as any).tts_pitch)
        // 发送音频给数字人引擎驱动口型（WebRTC 视频已静音）
        dh.speakAudio(audioBlob)
        // 本地播放同一份 MP3，用 currentTime 驱动字幕，完美同步
        playSynced(reply, audioBlob)
      } catch (ttsErr) {
        console.error('[RealHuman] Edge-TTS failed, fallback to browser TTS:', ttsErr)
        stopPlayback()
        // 降级：仅浏览器语音合成（不调 dh.speak，避免 OpenTalking 内建 TTS 产生双音频）
        const voice = findBestVoice(persona.voice) || findFallbackVoice()
        if (voice) {
          const u = new SpeechSynthesisUtterance(reply)
          u.voice = voice; u.lang = 'zh-CN'
          u.rate = persona.browser_rate; u.pitch = persona.browser_pitch
          u.onstart = () => {
            // 用文字驱动数字人口型（OpenTalking 内建 TTS → muted, 仅驱动口型）
            dh.speak(reply)
          }
        ...
        const interval = setInterval(() => {
          ...
          const lines = shown.split(/[。\n]/).filter(Boolean)
          setSubtitle(lines.slice(-2).join('。') || shown)
      ...
      setSubtitle('抱歉，我暂时无法回答')
      setReplying(false)
    }
  }, [dh, playSynced, stopPlayback, persona])
