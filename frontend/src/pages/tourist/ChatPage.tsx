...
export default function ChatPage() {
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    const controller = chatApi.sendStream(
      { message: text.trim(), scenic_spot: 'lingshan', session_id: sessionIdRef.current,
        persona_name: p.name, persona_role: p.role, persona_style: p.style },
      (token: string) => {
  ...
  const headerBar = (
    <div style={{
      ...
      {/* 左上角头像 → 使用角色照片 */}
      <div style={{ width: 40, height: 40, borderRadius: '50%', overflow: 'hidden', background: persona.color + '20', flexShrink: 0 }}>
        {persona.image ? (
          <img src={persona.image} alt={persona.name} style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center top' }} />
        ) : (
          <span style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24 }}>{persona.emoji}</span>
        )}
      </div>
      <div style={{ flex: 1 }}>
          ...
          <div key={i} className={m.role === 'user' ? 'msg-user-in' : 'msg-ai-in'} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start', marginBottom: 12 }}>
            {m.role !== 'user' && (
              <div style={{
                ...
                marginRight: 6, flexShrink: 0, marginTop: 2, overflow: 'hidden',
              }}>
                {m.role === 'error' ? (
                  ...
                  <span style={{ color: '#fff', fontSize: 16 }}>{persona.emoji}</span>
                )}
              </div>
            )}
